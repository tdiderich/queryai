...
def run_standard_search_with_relative_lookback(query, platform_aliases, relative_lookback_minutes):
    path = 'search'
...
def check_async_search_status(tracking_id):
...
def run_async_search_with_relative_lookback(query, platform_aliases, relative_lookback_minutes):
    path = 'search_async'
    currentEpochTime = datetime.utcnow()
    relativeEpochTime = currentEpochTime - timedelta(minutes=relative_lookback_minutes)
    payload = {
...
if __name__ == '__main__':
    ...
    search_results = run_async_search_with_relative_lookback(query, platform_aliases, 30)
